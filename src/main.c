...
/* STORAGE */
volatile float
    record_voltage[RECORD_NUMBER]; // records to hold "X", for "Y" values
  ...
  while (1) {
    ...
    case 5:
      if (nextion_read_action() == 0x0B) {
        PAGE_NUMBER = 6;
        // increases duty cycle for PWM by 1 each second and measures RPM,
        // Voltage, Current and pressure on FSR
        ...
        for (int i = 0; i < RECORD_NUMBER; i++) {
          ...
          record_force[i] = adc_to_voltage(
              adc_read()); // value from 0 - (2^16)-1 (max is 5V). 1 =
